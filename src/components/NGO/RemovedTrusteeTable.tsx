...
interface RemovedTrusteeProps {
  ngoDetail: any;
}

const RemovedTrustee: React.FC<RemovedTrusteeProps> = ({ ngoDetail }) => {
  ...
  return (
    <BodyStyle>
      <Box className="top-userdata" style={{ height: "100%" }}>
        ...
        {!isEmpty(ngoDetail?.removed_trustee) ? (
          <CTable
            tableStyle={{
            ...
            row={ngoDetail?.removed_trustee}
            column={columns}
            rowCount={ngoDetail?.removed_trustee?.length}
            listsLoad={false}
