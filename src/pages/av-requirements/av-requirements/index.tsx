...
const RequirementList = () => {
  ...
  const [requirementList, setRequirementList] = React.useState<any>({
    lists: [],
    listsLoad: true,
  });
  ...
  //API For get a & vrequirements list
  async function getRequirements(nData) {
    setRequirementList((e) => ({ ...e, listsLoad: true }));
    try {
      const res = await getApiData(`av-requirements/list`, nData);
      ...
      if (res.success) {
        ...
        setRequirementList({ lists: [], listsLoad: false });
  ...
  //API For delete a & v requirements list
  async function deleteList(item: any) {
    setDltLoad(true);
    try {
      const id = item;
      const res = await getApiData(
        `av-requirements/delete/${id}`,
        "",
        "DELETE"
      );
      if (res.success) {
  ...
  //API For create/update a & v requirements list
  async function submitData(data: any) {
    setBtnLoad(true);
    const id = selectedItem?.id;
    let url = "";
    if (type === "edit") {
      url = `av-requirements/update/${id}`;
    } else {
      url = `av-requirements/create`;
    }
    try {
  ...
  return (
    ...
    >
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          <BreadWrapper>
            <Breadcrumbs aria-label="breadcrumb" sx={{ margin: "20px 0px" }}>
              ...
              <Typography color="text.primary">A & V Requirements</Typography>
            </Breadcrumbs>
          </BreadWrapper>
        </div>
      </AppsHeader>
      ...
      <CTable
        ...
        row={requirementList?.lists}
        column={columns}
        rowCount={pagination.total}
        page={pagination.page}
        listsLoad={requirementList?.listsLoad}
        checkboxSelection={false}
        onSelectionModelChange={() => console.log("row select")}
        sortModel={sortModel}
        getApiCall={getRequirements}
      ...
      <DialogSlide
        ...
        dialogTitle={
          type === "edit"
            ? "Update A & V Requirements"
            : "Add New A & V Requirements"
        }
      >
        <Box sx={style}>
          <Formik
