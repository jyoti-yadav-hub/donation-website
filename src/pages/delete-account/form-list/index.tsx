...
const DeleteAccount = () => {
  ...
  //API For get delete account form list
  async function getFormList(nData) {
    setFormList((e) => ({ ...e, listsLoad: true }));
    try {
      nData.type = location?.pathname === "/report-drive" ? "drive" : null;
      let url = "";
      if (location?.pathname === "/report-drive") {
        url = `report/list`;
      } else {
        url = `delete-account/list`;
      }
  ...
  //API For create/update delete account form
  async function submitData(data: any) {
    if (isEmpty(feilds)) {
      ...
      const fData = {
        ...
        type: location?.pathname === "/report-drive" ? "drive" : null,
      };
      setBtnLoad(true);
      ...
      if (location?.pathname === "/report-drive") {
        ...
        if (type === "edit") {
          url = `delete-account/update/${id}`;
        } else {
          url = `delete-account/create`;
        }
      }
      try {
        ...
        >
          ...
          <AccordionDetails sx={{ p: "0px 15px 0px 0px" }}>
            <Grid container spacing={2} key={ques.id} style={mainSection}>
              ...
              {ques.other && (
                <Grid item xs={12}>
                  {optionForm(ques.other_input_data, i, indexes)}
                </Grid>
              )}
