...
const validationSchema = yup.object({
  description: yup
    .string()
    .required("Privacy Policy description is required")
    .min(3, "Privacy Policy description should be 3 chars minimum.")
...
const PrivacyPolicyList = () => {
  ...
  const [privacyPolicyList, setPrivacyPolicyList] = React.useState<any>({
    lists: [],
    listsLoad: true,
  });
  ...
  //API For get Privacy Policy list
  async function getPrivacyPolicy(nData) {
    setPrivacyPolicyList((e) => ({ ...e, listsLoad: true }));
    try {
      const res = await getApiData(`privacy-policy/list`, nData);
      if (res.success) {
        ...
        setPrivacyPolicyList({ lists: [], listsLoad: false });
  ...
  //API For delete Privacy Policy list
  async function deleteList(item: any) {
    setDltLoad(true);
    try {
      const id = item;
      const res = await getApiData(`privacy-policy/delete/${id}`, "", "DELETE");
      if (res.success) {
        setOpen(false);
        getPrivacyPolicy({
          page: 1,
  ...
  //API For create/update Privacy Policy list
  ...
  return (
    ...
    >
      ...
      <CTable
        ...
        row={privacyPolicyList?.lists}
        column={columns}
        rowCount={pagination.total}
        page={pagination.page}
        listsLoad={privacyPolicyList?.listsLoad}
        checkboxSelection={false}
        onSelectionModelChange={() => console.log("row select")}
        sortModel={sortModel}
        getApiCall={getPrivacyPolicy}
      ...
      <DialogSlide
        ...
        onClose={handleClose}
        dialogTitle={
          type === "edit" ? "Update Privacy Policy" : "Add New Privacy Policy"
        }
      >
        <Box sx={style}>
          <Formik
      ...
      <AppConfirmDialog
        ...
        dialogTitle="Delete Privacy Policy"
      />
    </AppsContainer>
  );
};
