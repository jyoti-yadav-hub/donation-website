...
const validationSchema = yup.object({
  description: yup
    .string()
    .required("Awards Prices description is required")
    .min(3, "Awards Prices description should be 3 chars minimum.")
...
const AwardsPricesList = () => {
  ...
  const [awardsPricesList, setAwardsPricesList] = React.useState<any>({
    lists: [],
    listsLoad: true,
  });
  ...
  //API For get awards-prices list
  async function getAwardsPrices(nData) {
    setAwardsPricesList((e) => ({ ...e, listsLoad: true }));
    try {
      const res = await getApiData(`awards-prices/list`, nData);
      if (res.success) {
        setAwardsPricesList({
        ...
        setAwardsPricesList({ lists: [], listsLoad: false });
  ...
  //API For delete awards-prices list
  async function deleteList(item: any) {
    setDltLoad(true);
    try {
      const id = item;
      const res = await getApiData(`awards-prices/delete/${id}`, "", "DELETE");
      if (res.success) {
        setOpen(false);
        getAwardsPrices({
          page: 1,
  ...
  //API For create/update awards-prices list
  async function submitData(data: any) {
    setBtnLoad(true);
    const id = selectedItem?.id;
    let url = "";
    if (type === "edit") {
      url = `awards-prices/update/${id}`;
    } else {
      url = `awards-prices/create`;
    }
    try {
  ...
  return (
    ...
    >
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          <BreadWrapper>
            <Breadcrumbs aria-label="breadcrumb" sx={{ margin: "20px 0px" }}>
              ...
              <Typography color="text.primary">Awards Prices</Typography>
            </Breadcrumbs>
          </BreadWrapper>
        </div>
      </AppsHeader>
      ...
      <CTable
        ...
        row={awardsPricesList?.lists}
        column={columns}
        rowCount={pagination.total}
        page={pagination.page}
        listsLoad={awardsPricesList?.listsLoad}
        checkboxSelection={false}
        onSelectionModelChange={() => console.log("row select")}
        sortModel={sortModel}
        getApiCall={getAwardsPrices}
      ...
      <DialogSlide
        ...
        onClose={handleClose}
        dialogTitle={
          type === "edit" ? "Update Awards Prices" : "Add New Awards Prices"
        }
      >
        <Box sx={style}>
          <Formik
      ...
      <AppConfirmDialog
        ...
        title="Are you sure you want to delete this Awards Prices?"
        dialogTitle="Delete Awards Prices"
      />
    </AppsContainer>
  );
};

export default AwardsPricesList;
