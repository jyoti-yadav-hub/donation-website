...
const validationSchema = yup.object({
  description: yup
    .string()
    .required("Rules Regulation description is required")
    .min(3, "Rules Regulation description should be 3 chars minimum.")
...
const RulesRegulationList = () => {
  ...
  const [rulesRegulationList, setRulesRegulationList] = React.useState<any>({
    lists: [],
    listsLoad: true,
  });
  ...
  //API For get Rules Regulation list
  async function getRulesRegulation(nData) {
    setRulesRegulationList((e) => ({ ...e, listsLoad: true }));
    try {
      const res = await getApiData(`rules/list`, nData);
      if (res.success) {
        setRulesRegulationList({
        ...
        setRulesRegulationList({ lists: [], listsLoad: false });
  ...
  async function getmycategories() {
    try {
      ...
      if (resp.success) {
        const categories = isArray(resp.data) ? resp.data : [];
  ...
  //API For delete Rules Regulation list
  async function deleteList(item: any) {
    setDltLoad(true);
    try {
      const id = item;
      const res = await getApiData(`rules/delete/${id}`, "", "DELETE");
      if (res.success) {
        setOpen(false);
        getRulesRegulation({
          page: 1,
  ...
  //API For create/update Rules Regulation list
  ...
  return (
    ...
    >
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          <BreadWrapper>
            <Breadcrumbs aria-label="breadcrumb" sx={{ margin: "20px 0px" }}>
              ...
              <Typography color="text.primary">Rules Regulation</Typography>
            </Breadcrumbs>
          </BreadWrapper>
        </div>
      </AppsHeader>
      ...
      <CTable
        ...
        row={rulesRegulationList?.lists}
        column={columns}
        rowCount={pagination.total}
        page={pagination.page}
        listsLoad={rulesRegulationList?.listsLoad}
        checkboxSelection={false}
        onSelectionModelChange={() => console.log("row select")}
        sortModel={sortModel}
        getApiCall={getRulesRegulation}
      ...
      <DialogSlide
        ...
        dialogTitle={
          type === "edit"
            ? "Update Rules Regulation"
            : "Add New Rules Regulation"
        }
      >
        <Box sx={style}>
          <Formik
      ...
      <AppConfirmDialog
        ...
        title="Are you sure you want to delete this Rules Regulation?"
        dialogTitle="Delete Rules Regulation"
      />
    </AppsContainer>
  );
};

export default RulesRegulationList;
